	...
	void deleteNode(T _nodeObject)
	{
		...
		if (_nodeNumber != -1)
		{
			...
			for (int i = 0; i < edge.size(); i++)
			{
				...
				if (edge[i].getBegin() >= _nodeNumber)
				{
					edge[i].setBegin(edge[i].getBegin() - 1);
				}
				if (edge[i].getEnd() >= _nodeNumber)
				{
					edge[i].setEnd(edge[i].getEnd() - 1);
				}
			}
		}
	}
...
int main()
{
	...
	while (true)
	{
		...
		cout << "9. Degree of the node" << endl;
		cout << "10. Iterator for nodes" << endl;
		cout << "11. Iterator for edges" << endl;
		cout << "12. Does node exist" << endl;
		cout << "13. Does edge exist" << endl;
